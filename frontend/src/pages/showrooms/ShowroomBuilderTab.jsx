...
const BLOCKS = [
    ...
    { type: 'collection_intro', icon: HelpCircle, label: 'Showroom Intro' },
    { type: 'product_grid', icon: LayoutGrid, label: 'Product Grid' },
    { type: 'contact', icon: Type, label: 'Get Info' },
]

export function ShowroomBuilderTab({ buyerMode = false, autoExport = false, onExportDone }) {
    const { id } = useParams()
    ...
    useEffect(() => {
        const loadShowroom = async () => {
            try {
                ...
                // Sync newly added products from showroom.items into the product_grid section
                ...
                console.error('Failed to fetch showroom:', _)
                toast.error('Failed to load showroom draft')
                navigate('/showrooms')
            } finally {
                setLoading(false)
            }
    ...
    useEffect(() => {
        if (printMode) {
            try {
                ...
                style.setAttribute('data-print-style', 'showroom-only')
                style.textContent = `
                  body.only-showroom-print { background: #fff !important; }
                  @page { size: A4; margin: 0.5in; }
                `
                document.head.appendChild(style)
                document.body.classList.add('only-showroom-print')
                ...
                if (!buyerMode) {
                    ...
                    if (root) {
                        ...
                        Array.from(root.children).forEach((el) => {
                            if (!el.querySelector?.('#showroom-print-root')) {
        ...
        if (printMode && showroom && sections.length > 0 && products.length > 0) {
            setTimeout(() => {
                try {
        ...
        return () => {
            if (printMode) {
                document.body.classList.remove('only-showroom-print')
                const s = document.querySelector('style[data-print-style="showroom-only"]')
                if (s) s.remove()
                try { (window).__CAT_PRINT_RESTORE__?.() } catch {}
            }
        }
    }, [printMode, showroom, sections, products, buyerMode])

    useEffect(() => {
        if (autoExport && showroom && !exporting) {
    ...
    const handleGeneratePdf = async () => {
        ...
        try {
            ...
            if (!buyerMode) {
                ...
                toast.message('Saving showroom changes...')
                const { updatedSections, newShowroomItems } = await syncDraftProductsToBackend(showroom.status === 'Published' ? 'ACTIVE' : 'DRAFT')
                await api.put('/showrooms/' + showroom.id, {
                    status: showroom.status,
                    themeColor: themeColor,
                    sections: updatedSections,
                    items: newShowroomItems
                })
                setShowroom(prev => ({ ...prev, themeColor, sections: updatedSections, items: newShowroomItems }))
            }
            ...
            await new Promise((resolve) => {
                setTimeout(async () => {
                    try {
                        ...
                        const element = document.getElementById('showroom-print-root') || document.querySelector('.showroom-canvas-container')
                        
                        if (!element) {
                            throw new Error("Could not find showroom layout")
                        }

                        const opt = {
                            margin:       [0.2, 0, 0.2, 0], // Slight top/bottom margin for breathing room
                            filename:     `${(showroom?.name || 'showroom').replace(/[^a-z0-9]/gi, '_').toLowerCase()}.pdf`,
                        ...
                        setPrintMode(false)
                        setExporting(false)
                        toast.success('Showroom PDF downloaded successfully')
                        resolve()
                    } catch (err) {
    ...
    const handleColorPick = (e) => {
        ...
        setThemeColor(hex)
        setShowroom(prev => ({ ...prev, themeColor: hex }))
    ...
    const syncDraftProductsToBackend = async (targetProductStatus) => {
                ...
                for (let j = 0; j < updatedItems.length; j++) {
                    ...
                    if (isFakeId(item.id) && (item.name || item.image || item.price)) {
                        try {
                            ...
                            console.error('Failed to auto-create product from showroom', err)
                        }
        ...
        return { updatedSections, newShowroomItems }
    }

    const handleSaveDraft = async () => {
        try {
            const { updatedSections, newShowroomItems } = await syncDraftProductsToBackend('DRAFT')
            await api.put('/showrooms/' + showroom.id, {
                status: 'Draft',
                themeColor: themeColor,
                sections: updatedSections,
            ...
            setShowroom(prev => ({ ...prev, status: 'Draft', themeColor, sections: updatedSections, items: newShowroomItems }))
    ...
    const handlePublish = async () => {
        try {
            const { updatedSections, newShowroomItems } = await syncDraftProductsToBackend('ACTIVE')
            await api.put('/showrooms/' + showroom.id, {
                status: 'Published',
                themeColor: themeColor,
                sections: updatedSections,
            ...
            setShowroom(prev => ({ ...prev, status: 'Published', themeColor, sections: updatedSections, items: newShowroomItems }))
            toast.success('Showroom published globally')
        } catch (error) {
            toast.error('Failed to publish showroom')
        }
    }

    const selectThemeColor = (color) => {
        setThemeColor(color)
        setShowroom(prev => ({ ...prev, themeColor: color }))
    }
    ...
    const getSectionDefaults = (type) => {
        switch (type) {
            ...
            case 'collection_intro': return { content: 'This showroom features our latest additions and curated pieces.' }
            case 'product_grid': return { content: 'The Collection', collectionName: 'Showroom' }
            case 'contact': return { content: 'Get Info', email: 'wholesale@brand.com', phone: '+1 800 555 1234' }
    ...
    const toggleSelectProduct = async (p) => {
        ...
        if (isSelected) {
            ...
            toast.success('Added to showroom selection', { description: `${p.name}` })
        }
        setSelectedProducts(next)
        try {
            if (!showroom) return
            const existingItems = showroom.items || []
            ...
            const updated = await api.put('/showrooms/' + showroom.id, { items: newItems })
            setShowroom(updated)
        } catch (_) {
            // non-blocking
        }
    ...
    const removeProductFromGrid = async (sectionId, itemIdx, itemId) => {
        ...
        // 2. Remove from backend showroom.items
        if (showroom && itemId && !String(itemId).startsWith('p')) {
            try {
                const newShowroomItems = (showroom.items || []).filter(it => it.id !== itemId)
                setShowroom(prev => ({ ...prev, items: newShowroomItems }))
                await api.put('/showrooms/' + showroom.id, { items: newShowroomItems })
                toast.success('Product removed from showroom')
            } catch (err) {
        ...
        <div className={cn("flex flex-col h-[calc(100vh-80px)] overflow-hidden -mx-4 -mt-6", printMode && "mx-0 mt-0 h-auto overflow-visible")}>
            ...
            <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200 bg-white shadow-sm z-10 w-full h-16 shrink-0">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" onClick={() => navigate('/showrooms')} className="-ml-2 text-gray-500 hover:text-black">
                        <ArrowLeft size={18} className="mr-1" /> Back
                    </Button>
                    <div className="h-5 w-px bg-gray-200"></div>
                    <div>
                        <h2 className="font-semibold text-[15px] leading-tight text-content-primary">{showroom?.name || 'Untitled Showroom'}</h2>
                        <p className="text-[11px] text-content-tertiary font-semibold tracking-wider uppercase flex items-center gap-2 mt-0.5">
                            {showroom?.type?.replace('-', ' ')} <span className="h-1 w-1 rounded-full bg-border-strong"></span>
                            <span className="text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded font-bold border border-amber-200/50">{showroom?.status || 'Draft'}</span>
                        </p>
                    </div>
                </div>
            ...
            <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200 bg-white shadow-sm z-10 w-full h-16 shrink-0">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" onClick={() => navigate('/buyer/showrooms')} className="-ml-2 text-gray-500 hover:text-black">
                        <ArrowLeft size={18} className="mr-1" /> Back
                    </Button>
                    <div className="h-5 w-px bg-gray-200"></div>
                    <div>
                        <h2 className="font-semibold text-[15px] leading-tight text-content-primary">{showroom?.name || 'Showroom'}</h2>
                        <p className="text-[11px] text-content-tertiary font-semibold tracking-wider uppercase flex items-center gap-2 mt-0.5">
                            {showroom?.type?.replace('-', ' ')}
                        </p>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    <Button className="shadow-md bg-brand text-white hover:bg-brand-dark" onClick={() => navigate(`/buyer/request-quote?showroomId=${id}`)}>Request Quote</Button>
                </div>
            </div>
            )}
            ...
            <div className="flex flex-1 overflow-hidden h-full">
                ...
                <div className={cn("flex-1 bg-[#F5F5F7] overflow-y-auto py-12 flex justify-center pb-40", printMode && "bg-white p-0 overflow-visible block pb-0") } onClick={() => setSelectedSectionId(null)}>
                    <div
                        id={printMode ? "showroom-print-root" : "showroom-print-root"}
                        className={cn(
                            printMode
                    ...
                    >
                        ...
                        {sections.length === 0 && (
                            <div className="absolute inset-0 top-32 flex flex-col items-center justify-center pointer-events-none opacity-40">
                                ...
                                <h3 className="text-lg font-bold text-gray-400">Your Showroom is Empty</h3>
